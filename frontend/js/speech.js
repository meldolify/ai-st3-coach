...
class SileroVADManager {
    ...
    try {
      ...
      this.vad = await vad.MicVAD.new({
        ...
        onSpeechStart: () => {
          ...
          // If AI is speaking and user starts talking, request interrupt
          if (this.aiIsSpeaking && this.onInterruptRequest) {
            console.log('[SILERO VAD] User interrupted AI');
        ...
        onSpeechEnd: async (audio) => {
          ...
          // Don't send if AI was speaking during this utterance (contaminated)
          if (this.aiIsSpeaking) {
            console.log('[SILERO VAD] Discarding - AI was speaking');
            return;
          }
        ...
        // VAD parameters tuned for voice agents
        positiveSpeechThreshold: 0.5,   // Confidence threshold for speech start
        negativeSpeechThreshold: 0.35,  // Threshold to end speech
  ...
  async sendAudioToWhisper(audioFloat32) {
    ...
    try {
      ...
      console.log(`[SILERO VAD] WAV conversion: ${Date.now() - startTime}ms, size: ${wavBlob.size} bytes`);

      // Convert to base64
      const base64Audio = await this.blobToBase64(wavBlob);
      console.log(`[SILERO VAD] Base64 conversion: ${Date.now() - startTime}ms`);
      ...
      this.websocket.send(JSON.stringify({
        ...
        sessionId: sessionId,
        audio: base64Audio,
        format: 'wav'  // Tell backend this is WAV format
      }));

      console.log(`[SILERO VAD] Audio sent to Whisper (total: ${Date.now() - startTime}ms)`);
...
// ============================================================================
// WHISPER RECOGNITION MANAGER (Legacy - kept as fallback)
// ============================================================================
