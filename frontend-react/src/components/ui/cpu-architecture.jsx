...
/**
 * CpuArchitecture — circuit-style SVG with travelling lights along masked paths.
 * Adapted from 21st.dev (svg-ui/cpu-architecture). Original was TSX; converted to JSX.
 *
 * Added: `hideChip` prop. When true, hides the central CPU rect, label text,
 * and connection rects so only the circuit lines + travelling lights remain.
 * Used as a backdrop behind the voice orb.
 *
 * Travelling lights are powered by `.cpu-architecture` + `.cpu-line-1..8`
 * + `@keyframes animation-path` declared in index.css.
 */
export function CpuArchitecture({
  ...
  return (
    <svg
      className={cn('text-organic-stone', className)}
      width={width}
      height={height}
      viewBox="0 0 200 100"
    >
      ...
      {/* Travelling lights along the masks */}
      ...
      {/* Centre chip — hidden via prop when used as a pure backdrop */}
      {!hideChip && (
        <g>
          {showCpuConnections && (
