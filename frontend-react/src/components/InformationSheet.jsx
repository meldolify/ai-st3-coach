    ...
    <div className="h-full flex flex-col overflow-hidden">
      ...
      <div className="px-5 pt-4 pb-3 border-b border-black/[0.06] shrink-0">
        <div className="flex items-center gap-2.5">
          ...
          <h2 className="font-display text-[15px] text-text-primary">{title}</h2>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-5 py-4">
        <dl className="space-y-3">
          {infoSheet.fields.map((field, i) => (
            ...
            >
              <dt className="text-[11px] font-medium text-text-muted uppercase tracking-wider mb-0.5">
                {field.key}
              </dt>
              <dd className="text-[13px] text-text-primary leading-relaxed">
                {field.value}
              </dd>
            </motion.div>
          ))}
              ...
              {infoSheet.images.map((img, i) => (
                <button
                  ...
                  className={cn(
                    ...
                    'hover:ring-2 hover:ring-accent/40 transition-all duration-200',
